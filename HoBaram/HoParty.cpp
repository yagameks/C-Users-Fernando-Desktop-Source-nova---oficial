...
void HoParty::resize()
{
	...
	//pHoPartyMsgBox.x = int((float)(smScreenWidth / 2 * 1.28f));
	//pHoPartyMsgBox.y = 10;
...
void HoParty::DrawPartyMessage(int x, int y)
{
	...
	if(MessageBoxOpenFlag == TRUE)
	{
		...
		dsTextLineOut(hdc, x + 10, y + 38, HoText_PartyMessage, lstrlen(HoText_PartyMessage));
...
int HoParty_KTJ::Draw()
{
	...
	for(i = qNameS; i < qNameE; i++)
	{
		...
		if(qN_Pressf && (((qN_Pressf - 8) + qNameS) == i))
			SetFontTextColor(hdc, RGB(255, 0, 0));
...
menu_frList:
	...
	if(cN_MenuN == 0)
	{
		...
		SetFontTextColor(hdc, RGB(255, 220, 0));
		...
		for(i = 0; i < 7; i++)
		{
			...
			{
				SetFontTextColor(hdc, RGB(255, 200, 100));
			}
			else
			{
				SetFontTextColor(hdc, RGB(255, 255, 255));
