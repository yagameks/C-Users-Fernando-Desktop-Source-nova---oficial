...
int HoLogin::Init()
{
	...
	SetFade();
	smInitColorZClip(21, 17, 14); // Loading apos selecionar personagem
	smRender.Color_R = 0;
	smRender.Color_G = 0;
	smRender.Color_B = 0;
	smRender.Color_A = 0;
...
int HoLogin::Load()
{
	...
	//Stage = smSTAGE3D_ReadASE("game\\GUI\\Opening\\Select\\Stage\\Stage.ASE", smStage);
...
{
	GameMapImage = LoadDibSurfaceOffscreen("game\\GUI\\Opening\\Loading\\0.bmp");
}
...
int HoLogin::DrawCharacterInfo()
{
	...
	if (DownUserCharacterInfo[MouseMoveCharacterNum].ClanFlag)
	{
		SetFontTextColor(hdc, RGB(230, 240, 255));
	...
	SetFontTextColor(hdc, RGB(255, 220, 100));
	char buffer[256];
	...
	SetFontTextColor(hdc, RGB(255, 250, 235));
