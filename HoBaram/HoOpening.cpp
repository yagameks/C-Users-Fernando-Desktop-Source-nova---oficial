...
char *szJoinWebUrl = "http://www.UnderWorld.com";
#endif
...
int HoOpening::Draw()
{
		...
		|| (CurrentWorkState == STATE_SERVER_SELECT && NextWorkState == STATE_LOGIN_INPUT))
	{
		...
		for(int index = 0; index < UserServerListInfoCount; index++)
		{
			...
			if(MouseDownServerIndex == -1)
			{
				if(pCursorPos.x > selectStartPosX && (pCursorPos.y > selectStartPosY + index*selectHeight) &&
					...
					SetFontTextColor(opHdc, RGB(250, 200, 0));
				else
					SetFontTextColor(opHdc, RGB(190, 190, 190));
			}
			else
			{
				if(MouseDownServerIndex == index)
		...
		if(oldFont)
			SelectObject(opHdc, oldFont);
		//d3ddev->lpDDSBack->ReleaseDC(opHdc);
	}
	...
	if(GetClick((smScreenWidth >> 1) - 76, 10, 152, 28))
	{
		dsDrawTexImage(iSettingsButton[1], (smScreenWidth >> 1) - 76, 10, 152, 28, 255);
		...
		dsDrawTexImage(iSettingsButton[0], (smScreenWidth >> 1) - 76, 10, 152, 28, 255);
	...
	if(hFont != NULL)
	{
		...
		if(bCaptureOpening)
		{
			...
			dsTextLineOut(opHdc, (smScreenWidth / 2) - 90, 20, localtime, strlen(localtime));
		...
		/*
		VERS? DO GAME
		wsprintf(buffer, "Vers?: 1010", Client_Version);
		dsTextLineOut(opHdc, 30, smScreenHeight - 50, buffer, strlen(buffer));*/
		...
		if(LoginBlendValue >= 255)
		{
			...
			if(hFocusWnd)
			{
				if(CaretCount < 16)
				{
					...
					if(LoginInputState == 0)
						dsTextLineOut(opHdc, startLogoX + startAccountAddX + 12 + End * 6 + 1, startLogoY + startAccountAddY + 20, "|", 1);
					else if(LoginInputState == 1)
						dsTextLineOut(opHdc, startLogoX + startAccountAddX + 12 + End * 6 + 1, startLogoY + startAccountAddY + 70, "|", 1);
				}
			...
			dsTextLineOut(opHdc, startLogoX + startAccountAddX + 13, startLogoY + startAccountAddY + 20, UserAccount + sp, len);
			len = lstrlen(UserPassword);
			if(len > DISP_LOGIN_LEN) len = DISP_LOGIN_LEN;

			for(int count = 0; count < len; count++)
				dsTextLineOut(opHdc, startLogoX + startAccountAddX + 13 + count * 6, startLogoY + startAccountAddY + 73, "*", 1);
	...
	{
		dsDrawTexImage(iSettingsBg, (smScreenWidth >> 1) - 279, (smScreenHeight >> 1) - 218, 559, 438, 255);

		dsDrawTexImage(iSettingsSave, ((smScreenWidth >> 1) + 185), ((smScreenHeight >> 1) - 218) + 405, 81, 22, 255);

		if(GetClick(((smScreenWidth >> 1) + 185), ((smScreenHeight >> 1) - 218) + 405, 80, 21))
		{
			dsDrawTexImage(iSettingsSave, ((smScreenWidth >> 1) + 185), ((smScreenHeight >> 1) - 218) + 405, 81, 22, 255);
			
			Settings::getInstance(this)->save();
			SettingsUI::getInstance()->close();
		}
		...
		SetBkMode(opHdc2, TRANSPARENT);
		SetFontTextColor(opHdc2, RGB(120, 120, 255));

		dsTextLineOut(opHdc2, ((smScreenWidth >> 1) + 129), ((smScreenHeight >> 1) - 110) - 42, "UnderWorld.com", strlen("UnderWorld.com"));
		dsTextLineOut(opHdc2, ((smScreenWidth >> 1) - 240), ((smScreenHeight >> 1) - 71) + 238, "UnderWorld, o melhor priston tale!", strlen("UnderWorld, o melhor priston tale!"));


		HDC opHdc = NULL;

		SetBkMode(opHdc, TRANSPARENT);
		SetFontTextColor(opHdc, RGB(120, 120, 255));
		...
		if(hFont != NULL)
		{
			HFONT oldFont = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) + 165), ((smScreenHeight >> 1) - 110) - 60, "UnderWorld", strlen("UnderWorld"));			

			HFONT Janela = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 252), ((smScreenHeight >> 1) - 119) - 51, "Op寤es da Janela", strlen("Op寤es da Janela"));

			HFONT LembrarID = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 70), ((smScreenHeight >> 1) - 119) - 51, "Deseja lembrar sua ID?", strlen("Deseja lembrar sua ID?"));

			HFONT Resolucao = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 248), ((smScreenHeight >> 1) - 71) - 51, "Resolu豫o", strlen("Resolu豫o"));

			HFONT visao = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 50), ((smScreenHeight >> 1) - 71) - 51, "Alcance de vis?", strlen("Alcance de vis?"));

			HFONT inverter = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 50), ((smScreenHeight >> 1) - 80) + 20, "Inverter a c?era?", strlen("Inverter a c?era?"));

			HFONT bitdeph = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 248), ((smScreenHeight >> 1) - 71) + 53, "Bit Deph", strlen("Bit Deph"));

			HFONT clima = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 50), ((smScreenHeight >> 1) - 71) + 62, "Ativar efeito de clima?", strlen("Ativar efeito de clima?"));

			HFONT grafico = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 248), ((smScreenHeight >> 1) - 71) + 113, "Gr?ico do UnderWorld", strlen("Gr?ico do UnderWorld"));

			HFONT som = NULL;
			oldFont = (HFONT)SelectObject(opHdc, hFont);
			dsTextLineOut(opHdc, ((smScreenWidth >> 1) - 50), ((smScreenHeight >> 1) - 71) + 108, "Efeitos sonoros", strlen("Efeitos sonoros"));
		...
		if(Settings::getInstance(true)->getWindowMode())
		{			
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 250) + 81 + 20, "Modo Janela", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 250) + 81 + 20, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWindowMode(false);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 250) + 81 + 20, "Modo Janela", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 250) + 81 + 20, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWindowMode(true);
			}
		}

		if(Settings::getInstance(true)->getRememberID())
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 250) + 81 + 20, "Lembrar ID", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 250) + 81 + 20, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setRememberID(false);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 250) + 81 + 20, "Lembrar ID", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 250) + 81 + 20, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setRememberID(true);
			}
		}

		if(Settings::getInstance(true)->getBitDepth() == 32)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 184 + 35, "32 bits", TRUE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 201 + 35, "16 bits", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 201 + 35, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setBitDepth(16);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 184 + 35, "32 bits", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 201 + 35, "16 bits", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 184 + 35, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setBitDepth(32);
			}
		}

		if(Settings::getInstance(true)->getTextureQuality() == 0)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 239 + 40, "Gr?ico Alto", TRUE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 256 + 40, "Gr?ico Baixo", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 256 + 40, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setTextureQuality(3);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 239 + 40, "Gr?ico Alto", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 256 + 40, "Gr?ico Baixo", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 239 + 40, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setTextureQuality(0);
			}
		}

		if(!Settings::getInstance(true)->getSight())
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "Curto", TRUE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "Longo", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setSight(true);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "Curto", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "Longo", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setSight(false);
			}
		}

		if(Settings::getInstance(true)->getWidth() == 800)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "800x600", TRUE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1024x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1280x960", FALSE);

			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "1366x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1600x900", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1920x1080", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(800);
				Settings::getInstance(true)->setHeight(600);
			}
			else if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(1024);
				Settings::getInstance(true)->setHeight(768);
			}
			else if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(1280);
				Settings::getInstance(true)->setHeight(960);
			}
			else if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 132 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(1400);
				Settings::getInstance(true)->setHeight(1050);
			}

			else if(GetClick(((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(1366);
				Settings::getInstance(true)->setHeight(768);
			}
			else if(GetClick(((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(1600);
				Settings::getInstance(true)->setHeight(900);
			}
			else if(GetClick(((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(1920);
				Settings::getInstance(true)->setHeight(1080);
			}
		}

		else if(Settings::getInstance(true)->getWidth() == 1024)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "800x600", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1024x768", TRUE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1280x960", FALSE);
		...
		else if(Settings::getInstance(true)->getWidth() == 1280)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "800x600", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1024x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1280x960", TRUE);
		...
		else if(Settings::getInstance(true)->getWidth() == 1366)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "800x600", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1024x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1280x960", FALSE);

			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "1366x768", TRUE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1600x900", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1920x1080", FALSE);
		...
		else if(Settings::getInstance(true)->getWidth() == 1600)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "800x600", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1024x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1280x960", FALSE);
			...
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "1366x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1600x900", TRUE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1920x1080", FALSE);
		...
		else if(Settings::getInstance(true)->getWidth() == 1920)
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "800x600", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1024x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1280x960", FALSE);
			...
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 81 + 20 + 15, "1366x768", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, "1600x900", FALSE);
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 120, ((smScreenHeight >> 1) - 218) + 115 + 20 + 15, "1920x1080", TRUE);
		...
		else
		{
			...
			if(GetClick(((smScreenWidth >> 1) - 279) + 30, ((smScreenHeight >> 1) - 218) + 98 + 20 + 15, 15, 15) && MouseButton[0])
			{
				Settings::getInstance(true)->setWidth(1024);
				Settings::getInstance(true)->setHeight(768);
			}
		...
		if(Settings::getInstance(true)->getCamInv())
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 186 + 10 - 20,
				"C?era Invertida", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 186 + 10 - 20, 15, 15)
				&& MouseButton[0])
			{
				Settings::getInstance(true)->setCamInv(false);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 186 + 10 - 20,
				"C?era Invertida", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 186 + 10 - 20, 15, 15)
				&& MouseButton[0])
			{
				Settings::getInstance(true)->setCamInv(true);
			}
		}

		if(Settings::getInstance(true)->getWeather())
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 15,
				"Clima ativo", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 15, 15, 15)
				&& MouseButton[0])
			{
				Settings::getInstance(true)->setWheater(false);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 15,
				"Clima desativado", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 15, 15, 15)
				&& MouseButton[0])
			{
				Settings::getInstance(true)->setWheater(true);
			}
		}

		if(Settings::getInstance(true)->hasSound())
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 60,
				"Som ativo", TRUE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 60, 15, 15)
				&& MouseButton[0])
			{
				Settings::getInstance(true)->setSound(false);
			}
		}
		else
		{
			DrawCheckBox(opHdc, ((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 60,
				"Som desativado", FALSE);

			if(GetClick(((smScreenWidth >> 1) - 279) + 228, ((smScreenHeight >> 1) - 218) + 203 + 10 + 60, 15, 15)
				&& MouseButton[0])
			{
				Settings::getInstance(true)->setSound(true);
			}
		}

		clickable = false;

		//Settings::getInstance(true)->save();
	}
	*/
		...
		&& ConnectError == ERROR_NONE)
	{
		...
		if(MouseDownFlag == FALSE)
		{
			if(MouseButton[0])
			{
				...
				else if(pCursorPos.x > (startLogoX + 230 + 98) &&
					pCursorPos.y > (startLogoY + 350 + 57) &&
					pCursorPos.x < (startLogoX + 230 + 146) &&
					pCursorPos.y < (startLogoY + 350 + 90))
				{
